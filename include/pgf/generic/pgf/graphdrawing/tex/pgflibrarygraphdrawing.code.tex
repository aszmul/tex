...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot-fuse/pgf/pgf/generic/pgf/graphdrawing/tex/pgflibrarygraphdrawing.code.tex,v 1.30 2013/03/03 22:13:12 tantau Exp $
...
\def\pgf@gd@handle@parameter#1#2#3{%
  ...
  \pgf@gd@parameter@stack@height\directlua{%
    local new, main = pgf.gd.interface.InterfaceToDisplay.pushOption('\luaescapestring{#1}',\pgfgdresult,\the\pgf@gd@parameter@stack@height)
    tex.print(new..'\luaescapestring{\relax}')
    if main then
...
\def\use@@gdlibrary#1{%
  ...
  \pgfutil@for\pgf@temp:=\pgf@list\do{%
    ...
    \else
      \pgfutil@IfFileExists{\directlua{tex.print((('pgf.gd.\pgf@temp.library'):gsub('\%.','/')))}.lua}
      {\directlua{require 'pgf.gd.\pgf@temp.library'} }
      {
      \pgfutil@IfFileExists{\directlua{tex.print((('pgf.gd.\pgf@temp.library'):gsub('\%.','/')))}.so}
      {\directlua{require 'pgf.gd.\pgf@temp.library'} }
      {
        \pgfutil@IfFileExists{\directlua{tex.print((('pgf.gd.\pgf@temp'):gsub('\%.','/')))}.lua}
        {\directlua{require 'pgf.gd.\pgf@temp'} }
        {
        \pgfutil@IfFileExists{\directlua{tex.print((('pgf.gd.\pgf@temp'):gsub('\%.','/')))}.so}
        {\directlua{require 'pgf.gd.\pgf@temp'} }
        {
          \pgfutil@IfFileExists{\directlua{tex.print((('\pgf@temp.library'):gsub('\%.','/')))}.lua}
          {\directlua{require '\pgf@temp.library'} }
          {
          \pgfutil@IfFileExists{\directlua{tex.print((('\pgf@temp.library'):gsub('\%.','/')))}.so}
          {\directlua{require '\pgf@temp.library'} }
          {
            \pgfutil@IfFileExists{\directlua{tex.print((('\pgf@temp'):gsub('\%.','/')))}.lua}
            {\directlua{require '\pgf@temp'} }
            {
            \pgfutil@IfFileExists{\directlua{tex.print((('\pgf@temp'):gsub('\%.','/')))}.so}
            {\directlua{require '\pgf@temp'} }
            {
              \PackageError{pgf}{Graph drawing library '\pgf@temp' not found}{}
            }}
          }}
        }}
      }}
