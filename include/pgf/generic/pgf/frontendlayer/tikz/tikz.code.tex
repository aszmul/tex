...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot-fuse/pgf/pgf/generic/pgf/frontendlayer/tikz/tikz.code.tex,v 1.101 2013/02/23 10:17:21 ludewich Exp $
      ...
      append after command = {%  
        \bgroup
          ...
          node [every label, 
                tikz@label@angle = #2,
                #1,
                tikz@label@post = \tikz@label@distance] {#3}
...
\def\tikz@fig ode{\pgfutil@ifnextchar a\tikz@test@also\tikz@normal@fig}
...
\def\tikz@node@also@opt[#1]{
  ...
  {\PackageError{tikz}{Syntax error in node also: ``('' expected.}{}%
    \tikz@scan@next@command}%
}
...
\def\tikz@ensure@dollar@catcode{%
  \ifnum\catcode`\$=3 %
  \else
    \PackageError{tikz}{Sorry, some package has redefined the meaning of the math-mode dollar sign. This is incompatible with tikz and its calc library and might cause unrecoverable errors}{}%
