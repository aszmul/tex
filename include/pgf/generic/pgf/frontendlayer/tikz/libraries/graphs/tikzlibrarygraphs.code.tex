...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot-fuse/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/graphs/tikzlibrarygraphs.code.tex,v 1.35 2012/12/30 00:20:48 tantau Exp $
...
\def\tikz@lib@layout@node@opt[#1]{%
  ...
  \pgfutil@ifnextchar\bgroup{\tikz@lib@layout@start}{\PackageError{graphs
      library}{Opening brace at beginning of sublayout expected}{}}%
}
...
\def\tikz@lib@layout@start#1#2\pgf@stop{%
  ...
  \else%
    \ifx\tikz@lib@graph@name\pgfutil@empty%
      \PackageError{graphs library}{Internal error: Node has no name, but it should!}{}
    \else
...
\def\tikz@lib@ensure@gd@loaded{\PackageError{graphs library}{You must say \string\usetikzlibrary{graphdrawing} to use the (sub)layout syntax}{}}
\fi
...
% the <connector> is one of the five connectors "->", "<-", "--",
% "<->", and "-!-", followed by optional options in square
